from pf_subsetter.file_io_tools import read_geotiff, write_array_to_geotiff, write_bbox
import numpy as np
...
class SubsetMask:
    ...
    def write_png_inset(self, filename):
        ...
        ds_geom = self.mask_tif.GetGeoTransform()

        yy, xx = np.where(self.mask_array == 1)
        ...
        shape_1 = min(self.mask_array.shape[1], max(xx) + 1 + n4) - max(0, min(xx) - n3)
        shape_0 = min(self.mask_array.shape[0], max(yy) + n2 + 1) - max(min(yy) - n1, 0)
        ...
        inSRS = self.mask_tif.GetProjection()
        ...
        lon_0, lat_0 = transform(Proj(inSRS_forPyProj), Proj(init='epsg:4326'),
        ...
        im1 = ax.imshow(np.ma.masked_where(top_mat == 0, top_mat), cmap=cmap,
                        vmin=0 - .5, vmax=6 + .5)
        # im1 = ax.imshow(np.ma.masked_where(top_mat==0,top_mat),cmap=cmap,norm=mpl.colors.Normalize(vmin = -0.5, vmax = 6.5))
